...
void Command::voice_words_Callback(std_msgs::msg::String::SharedPtr msg){
	...
	if (str1 == str2){
		...
		system(WHOLE.c_str());
		cout<<"好的：小车前进"<<endl;
		cmd_msg.data = "order=1";
		cmd_vel_string_pub->publish(cmd_msg);
	}
	else if (str1 == str3){
		WHOLE = head + audio_path + "/car_back.wav";
		system(WHOLE.c_str());
		cout<<"好的：小车后退"<<endl;
		cmd_msg.data = "order=2";
		cmd_vel_string_pub->publish(cmd_msg);
	}
	else if (str1 == str4){
		...
		system(WHOLE.c_str());
		cout<<"好的：小车寻找声源"<<endl;
		cmd_msg.data = "wake_angle=" + to_string(angle);
		cmd_vel_string_pub->publish(cmd_msg);
	}
	else if (str1 == str9){
...
: rclcpp::Node(node_name){
	...
	/***小车控制话题发布者创建***/
	cmd_vel_string_pub = this->create_publisher<std_msgs::msg::String>("cmd_vel_string", 10);
	/***离线命令词识别结果话题订阅者创建***/
	...
	/***寻找语音开启标志位话题订阅者创建***/
	angle_msg_sub = this->create_subscription<std_msgs::msg::UInt32>(

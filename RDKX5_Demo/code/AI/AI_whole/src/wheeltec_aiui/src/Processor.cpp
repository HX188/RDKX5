...
void alsaStart()
{
    ...
    if (!audioListeningThread)
    {
        ...
        audioListeningThread->run();//单开一个线程采集音频
    }
}
...
/**
 * 从文件读音频写入SDK，即用文件数据模型实时录音数据。 用于验证
 *
 * @param repeat
 */
 ...
 * 注：新版本SDK都只支持esr。
 */
void buildEsrGrammar()  //读取离线命令词识别语法文件
{
 ...
 * 同步动态实体。
 */
void syncSchemaData(int type = AIUIConstant::SYNC_DATA_SCHEMA)//让 AIUI 能识别你自己的联系人、词条、设备名等，实现“个性化语音识别”。
{
...
#ifdef AIUI_V2
/**
 * 同步所见即可说的内容。让 AIUI 能识别你当前场景下的“可说内容”，比如你在一个展厅，面前有多个展品，AIUI 能识别你说的“介绍一下这个展品”。
 */
            ...
            if (sub == "iat") {
                ...
                if (reader.parse(resultStr, resultJson, false)) {
                    ...
                    if (isLast) {
                        ...
                        node->chat_words_pub->publish(msg);
                ...
                if (reader.parse(resultStr, resultJson, false)) {
                    ...
                    msg2.data = mIatTextBuffer;
                    node->chat_words_pub->publish(msg2);
                    mIatTextBuffer.clear();
                }
            }
        } break;

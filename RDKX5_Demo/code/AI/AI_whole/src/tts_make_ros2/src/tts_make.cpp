...
using namespace std;
std::string AUDIO_PATH = "/home/sunrise/AI/AI_whole/src/tts_make_ros2/audio";
std::string WHOLE;
std::string head = "aplay ";
pid_t play_pid = -1; // 全局变量，保存子进程 pid
...
int TTS::init()
{
	...
	WHOLE = AUDIO_PATH + "/voice_control.wav";
	play_audio(WHOLE);
...
: rclcpp::Node(node_name,options){
    ...
    "/awake_flag", 10, std::bind(&TTS::awake_flag_callback, this, std::placeholders::_1));
...
void TTS::awake_flag_callback(const std_msgs::msg::Int8::SharedPtr msg) {
    if (msg->data) { // 如果收到 true，可以根据需要调整
        stop_audio();
        RCLCPP_INFO(this->get_logger(), "收到，已停止音频播放");
    }
}

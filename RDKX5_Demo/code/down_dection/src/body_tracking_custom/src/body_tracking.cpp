...
TrackingManager::TrackingManager() {
  ...
  cmd_vel_string_publisher_ = param_node_->create_publisher<std_msgs::msg::String>(
    "/cmd_vel_string", 10);
...
void TrackingManager::UpdateTrackAngle() {
  ...
  cv::Point2f fit_center(track_cfg_.img_width / 2, track_cfg_.img_height);//底部中心
  cv::Point2f start_pt(track_cfg_.img_width, track_cfg_.img_height);//右下角
  cv::Point2f end_pt(
      (track_info_.present_rect[0] + track_info_.present_rect[2]) / 2,
      (track_info_.present_rect[1] + track_info_.present_rect[3]) / 2);//目标中心
...
void TrackingManager::DoRotateMove() {
  ...
  if (do_move || do_rotate) {
    ...
    // --- Start of added code ---
    if (do_move && !track_info_.has_face_head && twist->angular.z == 0) {
        twist->linear.x = 0; // Set linear.x to negative for backward movement
        RCLCPP_WARN(rclcpp::get_logger("TrackingManager"),
                    "Track has no face/head, moving backward with step: %f",
                    track_info_.move_step);
    }
    // --- End of added code ---
    ...
    // }
    // --- Start of added code ---
    if (cmd_vel_string_publisher_) {
        std_msgs::msg::String string_msg;
        std::stringstream ss;
        ss << "speed=" << std::fixed << std::setprecision(2) << twist->linear.x
           << ",rotate=" << std::fixed << std::setprecision(2) << twist->angular.z;
        string_msg.data = ss.str();
        cmd_vel_string_publisher_->publish(string_msg);
        RCLCPP_INFO(rclcpp::get_logger("TrackingManager"), "Published cmd_vel string: %s", string_msg.data.c_str());
    }
    // --- End of added code ---
  } else {
    CancelMove();
...
bool TrackingManager::TrackingSwitchWithVision() {
  ...
  if (abs(track_info_.robot_x_positive_radian_with_track_ * 180 / PI) <
    ...
    if (body_rect_to_top > track_cfg_.img_height * 0.5) {
      ...
      move_step_ratio = 1.0;
    } else if (body_rect_to_top > track_cfg_.img_height * 0.05) {
      move_step_ratio = 0.5;
    }
  }

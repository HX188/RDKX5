...
std::string gestureToString(inference::gesture_type g) {
    switch (g) {
        case inference::gesture_type::ThumbUp: return "order=1";
        case inference::gesture_type::Palm: return "order=2";
        case inference::gesture_type::ThumbLeft: return "order=4";
        case inference::gesture_type::ThumbRight: return "order=3";
        // ... 其他手势 ...
        default: return "";
    };
}
...
HandGestureDetNode::HandGestureDetNode(const std::string& node_name,
  ...
  gesture_value_pub_ = this->create_publisher<std_msgs::msg::String>("/cmd_vel_string", 10);
}
    ...
    if (target_has_hand &&
      ...
      if (gesture_res && !gesture_res->gesture_res_.empty()) {
        for (const gesture_type& res : gesture_res->gesture_res_) {
          ...
          int current_gesture = static_cast<int>(res);

          // 记录历史
          gesture_history_.push_back(current_gesture);
          if (gesture_history_.size() > 3) {
              gesture_history_.pop_front();
          }

          // 判断是否连续三帧相同
          if (gesture_history_.size() == 3 &&
              gesture_history_[0] == gesture_history_[1] &&
              gesture_history_[1] == gesture_history_[2]) {
              // 与上次确认的不同才发布
              if (last_confirmed_gesture_ != current_gesture) {
                  std_msgs::msg::String gesture_msg;
                  gesture_msg.data = gestureToString(res);
                  if (!gesture_msg.data.empty()) {
                  gesture_value_pub_->publish(gesture_msg);
                  }
                  last_confirmed_gesture_ = current_gesture;
              }
          }
          //  // 发布数值
          // std_msgs::msg::Int32 gesture_msg;
          // //gesture_msg.data = static_cast<int>(gesture_res->gesture_res_.front());
          // gesture_msg.data = gestureToString(res);
          // if (gesture_msg.data != NULL) {
          // gesture_value_pub_->publish(gesture_msg);
          // }
